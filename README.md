...
Status: **runtime + owned classifier + feedback + law benchmark (verified) + tenant context** — see [`docs/prismguard-design.md`](docs/prismguard-design.md) (v0.5), [`docs/guard-model-training.md`](docs/guard-model-training.md), and [`docs/tenant-context.md`](docs/tenant-context.md).
...
- **Domain packs**: law verified; healthcare/finance overlays only (no valid benchmark yet) — `prismguard init --domain law` or `PRISMGUARD_DOMAIN=law`.
...
- **Benchmark**: 4-stack law harness (CPL/CGL/LGL/LPL) — `benchmark/law/`. **Authoritative results:** `benchmark/law/results/verified/`. Run: `python -m benchmark.law.run_local_benchmark --output-dir benchmark/law/results/verified`. Repro check: `python scripts/compare_repro_runs.py run-a run-b`.
