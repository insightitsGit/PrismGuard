...
def run_user_verify(*, skip_runtime: bool = False) -> VerifyReport:
    ...
    gm = checker._guard_model  # noqa: SLF001
    if gm is not None and not gm.is_ready:
        report.add("classifier_ready", False, "install prismguard[guard-model] and bundled ONNX artifacts")
        return report
    ...
    attack_fails = 0
    for text in USER_VERIFY_ATTACK:
        if checker.check(text).decision != "block":
            attack_fails += 1
    report.add(
        "fresh_attacks",
        attack_fails == 0,
        f"{len(USER_VERIFY_ATTACK) - attack_fails}/{len(USER_VERIFY_ATTACK)} block",
    )
    return report
