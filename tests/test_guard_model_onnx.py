...
from prismguard.models.onnx_classifier import (
...
def test_dynamic_pad_length_caps_and_aligns() -> None:
    assert _dynamic_pad_length(1, max_length=256) == 8
    assert _dynamic_pad_length(9, max_length=256) == 16
    assert _dynamic_pad_length(250, max_length=256) == 256
    assert _dynamic_pad_length(300, max_length=256) == 256


def test_onnx_encode_uses_dynamic_padding(tmp_path: Path) -> None:
    ...
    assert prediction.details["sequence_length"] == 8
    mock_session.run.assert_called_once()
    args = mock_session.run.call_args[0][1]
    assert args["input_ids"].shape[1] == 8


def test_onnx_classifier_predict_maps_logits(tmp_path: Path) -> None:
    artifact_dir = tmp_path / "prism-pi-test2"
    artifact_dir.mkdir()
    write_model_card(
        artifact_dir,
        ModelCard(
            model_id="prism-pi-test2",
            version="1",
            architecture="sequence_classification",
            max_length=8,
            injection_label=1,
